...
│         ├── get_urban_boundary.py                                        # Extract and generate urban built-up boundary
...
| `CityPanel.csv` | Wide-format LPM sensitivity data (150 cities × 1128 columns: 120 base + 1008 sensitivity). Full 7×7 grid of (x0, k) perturbations at 70%–130%. Column suffix: `_x0{pct}%_k{pct}%` (e.g. `_x080%_k130%`). Reshaped to long format (7350 rows) on the fly by SFig2. | SFig2 |

### Intermediate Input

| File | Generated by | Consumed by |
|------|-------------|-------------|
| `IncludingLogData.csv` | `STable18_tranformation.R` — applies shifted-log transforms to skewed variables in `merged_V5_final.csv` | Figure4, Figure5, SFig4_Spatial_vs_NC, STable8_Robustness_ZonalBehavior |
...
| **6** | `get_*.py` | Run python scripts under `main/python/geometric` starting with `get_`. |
